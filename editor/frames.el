...
(defun move-buffer-to-other-frame ()
  ...
  (when (<= (length (frame-list)) 2)
    (let* ((current-frame (window-configuration-frame (current-window-configuration)))
	   (current-frame-index (position current-frame (frame-list)))
	   (current-window (frame-selected-window current-frame))
	   (current-buffer (window-buffer current-window))
	   (other-frame-existed (= (length (frame-list)) 2))
	   (other-frame-index (- 1 current-frame-index))
	   (other-frame (if (= (length (frame-list)) 1)
			    (progn
	   ...
	   (other-window (frame-selected-window other-frame)))
